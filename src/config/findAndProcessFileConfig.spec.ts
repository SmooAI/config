...
vi.mock('./getCloudRegion', () => ({
    getCloudRegion: vi.fn(() => ({ provider: 'aws', region: 'us-east-1' })),
}));
...
describe('findAndProcessConfig', () => {
    ...
    it('returns empty object if findConfigDirectory fails', async () => {
        ...
        await expect(findAndProcessFileConfig(configSchema)).rejects.toThrow('Could not find the directory where the config files are located.');
    });
    ...
    it('throws if default.ts is missing', async () => {
        ...
        await expect(findAndProcessFileConfig(configSchema)).rejects.toThrow(/Could not find required default config file in/);
    });
    ...
    it('throws if importing a config file fails', async () => {
        ...
        await expect(findAndProcessFileConfig(configSchema)).rejects.toThrow('Syntax error in default.ts');
    });
    ...
    it('merges config files in the correct order (happy path)', async () => {
        ...
        const { config: finalConfig } = await findAndProcessFileConfig(configSchema);
